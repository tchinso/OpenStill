...
- `openstill-export` v4 JSON 가져오기/내보내기(v2·v3도 호환): 최대 5,000개 추적, 파일 선택 상한 없음(브라우저 자원 범위 내). 내보내기는 32 MB 미만의 번호가 붙은 JSON 파일로 분할하며, 큰 단일 추적도 JSON 조각으로 나누어 여러 파일을 한 번에 불러올 수 있음(조각이 있는 경우 모든 part를 함께 선택)
